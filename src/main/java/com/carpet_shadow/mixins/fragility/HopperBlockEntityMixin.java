...
public abstract class HopperBlockEntityMixin {
    ...
    private static ItemStack extract_fix_entity(ItemStack instance, Operation<ItemStack> original) {
        if (CarpetShadowSettings.shadowItemTransferFragilityFix && ShadowItem.fromItemStack(instance).carpet_shadow$hasShadowId())
            return instance;
    ...
    private static void extract_fix_storage2(Inventory instance, int i, ItemStack itemStack, Operation<Void> original) {
        if (CarpetShadowSettings.shadowItemTransferFragilityFix && ShadowItem.fromItemStack(itemStack).carpet_shadow$hasShadowId())
